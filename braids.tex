...
Both \Verb+crossing convention+ and \Verb+set symbols+ take an option, which is one of \Verb+over+, \Verb+under+, \Verb+down+, or \Verb+up+; in this list \Verb+over+ and \Verb+down+ are synonyms, as are \Verb+under+ and \Verb+up+.
...
The \Verb+crossing convention+ keys set whether the standard crossings are over or under (\Verb+down+ and \Verb+up+ are synonyms for if you prefer to think of the braid as travelling down or up the page).
...
But for the other symbols, such as \(s_{1-4}\), then these achieve different things.
With the \Verb+crossing convention+ set to \Verb+under+ then \(s_{1-4}\) still moves each strand to the right.
But with \Verb+set symbols+ set to \Verb+under+ then each strand now moves to the left.

To see the effect of these options, consider \(s_{1,2,3}\).
...
\end{tabular}
\caption{Equivalences of \(s_{1,2,3}\)}
\label{tab:overunder}
\end{center}
