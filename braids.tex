...
This key is depreciated and is only kept for backwards compatibility, the following key should be used in its place.
...
If a \Verb+name prefix+\footnote{This is a TikZ key that appends a prefix to every node and pic name in its scope.
See the TikZ/pgf documentation for more details.} is in place, the coordinate names look like the following:
%
\begin{verbatim}
...
This node does not actually exist, so -- for example -- it can't directly be used to draw a border, but TikZ behaves afterwards as if it does exist.
So in particular its anchors can be used to define locations around the edge of the braid.
