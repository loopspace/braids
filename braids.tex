...
This default behaviour can be reversed by using the \Verb+set crossings+ or \Verb+flip crossings+ keys (described in a moment).
%
\begin{enumerate}
...
Both are in the \Verb+/tikz/braid/+ namespace.

\DescribeMacro{set crossings} The key \Verb+set crossings+ can be used to set the default behaviour of a braid element such as \Verb+s_1+.
If set to \Verb+over+ (the default) then \Verb+s_1+ is strand one going over strand two, when viewed as going down the page.
If set to \Verb+under+ then \Verb+s_1+ is strand one going under strand two, when viewed as going down the page.
Alternative options are \Verb+down+ as a synonym for \Verb+over+ and \Verb+up+ for \Verb+under+.

This switch propagates to superscripts, so that \Verb+s_1^{-1}+ should then be viewed as inverting the default behaviour.

\DescribeMacro{flip crossings} This key toggles the default behaviour of crossings.
...
\pic at (0,0) {braid={s_1}};
\pic[braid/set crossings=under] at (2,0) {braid={s_1}};
\end{tikzpicture}
\end{example}
