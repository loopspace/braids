...
\Verb+\pic[options] (name) at (coordinate) {braid={braid-word}};+
...
For example, \Verb+s_1-s_2+.

\item To draw a \emph{floor}, precede the braid element by a vertical line, as in \Verb+s_1 | s_2+.
The floor is a rectangular path which is drawn behind the braid and can be styled with the keys \Verb+braid/every floor+ and \Verb+braid/floor <n>+.
The \Verb+n+ is the crossing number.
...
When a floor is requested behind a crossing, it is rendered as a rectangular path.
These keys control how the floor is displayed.
...
\begin{tikzpicture}
\pic[
  braid/every floor path/.style={
    to path={ (\tikztostart |- \tikztotarget) -- (\tikztotarget) (\tikztostart) -- (\tikztostart -| \tikztotarget)}
  },
...
\node[at=(braid-rev-3-s),pin=north east:strand 3 start (from bottom)] {};
\node[at=(braid-rev-3-e),pin=south east:strand 3 end (from bottom)] {};
\end{tikzpicture}
\end{center}
\end{example}
...
\section{Original Package Usage}
